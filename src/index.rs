    ...
    impl<'a> Visitor for IndexBlockVisitor<'a> {
        fn visit_transaction(&mut self, tx: &bsl::Transaction) -> core::ops::ControlFlow<()> {
            ...
            for i in parsed_tx.input.iter() {
                ...
                // bytes.reverse();
                // println!("outpoint {}:{}", i.previous_output.txid.to_string(), i.previous_output.vout);

                // let bytes2: Vec<u8> = hex::decode(txid.to_string().as_str()).unwrap();

                // println!("encode::serialize {:?}", bytes);
                // println!("hex::decode       {:?}", bytes2);
            ...
            let path = "/home/node/Documents/smallest_outpoints.txt";
